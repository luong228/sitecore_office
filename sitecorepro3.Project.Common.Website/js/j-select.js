...
$selectDroplist_UI = function (jEl, options) { //jEl = jquery element
    ...
    this.setupDropListUI = function () {
            ...
            if ( this.tagName.toLowerCase() == "optgroup" ) {
                ...
                el.each(function () {
                    ...
                    optgroup.find("option").each(function (index) {
                        ...
                        var title = self.attr("title") || self.text();
                        if ( self.attr("value") == "null" ) {
                            optgroup_elsubUL.append("<li class=\"SelectUITitle\" value=\"" + parseInt(count + index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + self.attr("label") + "\">" + self.text() +"</a></li>");
                        }
                        else if ( this.getAttribute("selected") == "selected" || this.getAttribute("selected") == true ) {
                            optgroup_elsubUL.append("<li class=\"Active\" value=\"" + parseInt(count + index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + self.attr("label") + "\">" + self.text() +"</a></li>");
                        }
                        else {
                            optgroup_elsubUL.append("<li value=\"" + parseInt(count + index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + self.attr("label") + "\">" + self.text() +"</a></li>");
                        }
                    });
                ...
                if ( el.attr("value") == "null" ) {
                    o.elUL.append("<li class=\"SelectUITitle\" value=\"" + parseInt(index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + el.attr("label") + "\">" + el.text() +"</a></li>");
                }
                else if ( this.getAttribute("selected") == "selected" || this.getAttribute("selected") == true ) {
                    o.elUL.append("<li class=\"Active\" value=\"" + parseInt(index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + el.attr("label") + "\">" + el.text() +"</a></li>");
                }
                else {
                    o.elUL.append("<li value=\"" + parseInt(index + offset+1) + "\"><a href=\"#\" title=\"" + title + "\" rel=\"" + el.attr("label") + "\">" + el.text() +"</a></li>");
                }
            }
        });
        ...
        if ( !o.select .attr("multiple") ) {
            ...
            if ( !o.select.attr("disabled") ) {
                o.droplistTITLE.text(title.replace(/&nbsp;/g,''));
                ...
                self.bind("click", function () {
                    if ( self.find("span.OptgroupLabel:first-child").length > 0 ) {
                        ...
                        if ( !o.select.attr("disabled") ) {
                            ...
                            o.droplistTITLE.text( self.find('a').html().replace(/&nbsp;/g,'') );
    ...
    o.elUL = jQuery("<ul title=\"" + (o.select.attr("title") != undefined ? o.select.attr("title") : "") + "\"></ul>");
    o.elUL.addClass(o.select.attr("class"));
    o.el.html(o.elUL);

    if ( !o.select .attr("multiple") ) {
        o.droplistTITLE = jQuery('<p><span class="ChangeWidthCombobox"></span></p>');
		...
		o.droplistTITLE = jQuery('span', o.elWrapper);
    }

    this.setupDropListUI();
	o.droplistTITLE.width(o.elWrapper.width()-50);
    /*end. <select>*/
